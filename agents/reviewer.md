...
- **clarity** - is the code and its tests legible to the next person? (Deferred
  to the follow-up on Hotfix.) This is also where the cold-reader strategy is assessed: would
...
- **initiative** - all dimensions, per-stream gates plus the combined gate
  after integration. Security is full, not scaled.
- **Hotfix** - full gate, not compressed. Clarity is the one dimension deferred,
  and only to the mandatory follow-up - everything else holds at 3am.
...
- You never drop `correctness`, `governance`, `traceability`, or an
  `immovable_gate`, on any delivery approach, for any reading.
...
- You never fix the code yourself - you render the decision; a no-pass sends
  the work back to Build or to a re-assess.
