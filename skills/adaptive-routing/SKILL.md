...
description: How to score the four context dimensions, compose and constrain a route, and keep an honest de-scope ledger. Triggers during Frame, whenever the Needle runs, and whenever a task is re-framed mid-flight.
---
...
- The common mistake: scoring blast radius by how hard the change is. A hard
  change can be contained; a trivial-looking change can be critical. Score the
  failure, not the work.

**Terrain** - *new code, or old code, and is the old code mapped?*
...
**Magnitude** - *the dimension humans get wrong most.*
- People under-estimate magnitude far more than they over-estimate it. When
...
- **Land** - trivial integration vs. coordinated merge; which backfills are owed.

Name the nearest reference route for shared vocabulary, then list deviations
...
- **The TDD strategy (S2) is suspended.** Red-before-green is the wrong
  ...
  when it composes a Spike route**; without it the hook will still block.
...
- **Nothing lands from a Spike.** The only exit that keeps code is *graduating*
  - re-framing into a real delivery route where guardrails G1–G3 apply in full.
...
- A question that can only be answered by touching irreversible surface
  ...
  routing guardrail floors force those to Expedition regardless of intent.
...
- **A skip with no justification is not allowed. If you cannot justify it, the
  ...
  "arbitrary."
- Do not copy a reference route's standing justification on autopilot. Confirm
  it actually holds for *this* task. Express's "Clarify collapsed - the one
  scenario is unambiguous" is only valid if the scenario really is unambiguous.
- Cap-driven reductions are **not** de-scopes. If the `critical` blast radius
...
- Expedition's ledger is empty by definition. If you are writing de-scope rows
  on an Expedition route, you have mis-composed.
...
Re-framing mid-task is a normal event, not a failure. Trigger it when:

- Build reveals the magnitude was under-read - a "small" change is unspooling
