---
name: tdd-discipline
description: The red→green→refactor cycle as Compass runs it - TDD as the default strategy satisfying the tested-before-ship guardrail, how test surface scales with risk, what "the failing test first" means, approach-awareness (suspended on a spike), and working under the pre-tool hook. Triggers while implementing, on every delivery approach.
---
