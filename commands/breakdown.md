...
Breakdown executes the topology that the design decided. It creates the
worktrees and assigns the agent swarm.
...
computed from `governance/routing-policy.yml` (the shapes' `topology`, the
policy `caps`):
- **pair** (2-3 streams) - larger features. One worktree per stream, one
  `builder` agent each, no dedicated orchestrator - the lead builder
  integrates.
- **swarm** (4+ streams) - initiatives. One worktree per stream, one
...
1. **Verify the count.** Cross-check the map's stream count against the
   topology and any policy `cap` recorded in `delivery-approach.md` (the CLI
...
Worktrees exist and match the distribution map; every stream has a builder
and an assigned scenario set; the orchestrator is in place (swarm). Next:
`/compass:implement`.
