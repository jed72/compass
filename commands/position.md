...
The `product-marketer` reading (see `routes/router.md` and the routing
...
`positioning.md` and `launch-readiness.md` exist; every claim names a backing
scenario; voice is consistent with the voice & positioning strategies in
`governance/`. The claims gate is then carried into `/compass:verify` and
enforced at `/compass:land`.
