...
2. **Read `route.md` and `task.yml` first.** `route.md` is the human-readable
   ...
   the CLI reads. Everything else is read in light of them. If `route.md` is a
   `--reframe` revision, read the latest revision.
3. **Read the artifacts in pipeline order** - `brief.md`, `spec.feature.md`,
   `clarifications.md`, `plan.md`, `distribution-map.md`, role artifacts,
...
If `route.md` does not explain the process from here, the Needle under-framed.
