...
1. **Write the brief.** From `templates/brief.md`, capture:
...
3. **Write `brief.md`** into `.compass/work/<task-slug>/`.
...
the `brief.md` artifact, and the **intent-fidelity gate** — the spec must be
checked against `brief.md` before Plan. Frame reads the brief as the *actual
outcome wanted*, not just the literal request.

## Gate

`brief.md` exists with all four sections real; it is consistent with the
