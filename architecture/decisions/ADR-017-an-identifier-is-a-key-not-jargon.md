...
every time after that.** Both halves are needed. Expanding on every
...
same treatment, and the receipt was changed in the same release to print a
...
observe what an agent says in a session, and claiming otherwise would be
one more check that cannot fail.
...
- `governance/strategies.md` `S7` - the cold reader, which now carries the
  rule.
- `governance/terminology.yml`, lines 21-23 - names the vocabulary as
  frozen, which is what makes changing the meaning of an entry an
  architectural decision rather than an edit.
