...
Compass v1 wrapped its two central ideas - process sized to the change,
...
is affordable, while the user base is small. A breaking rename is possible
only once: a v3 rename is not available for years. So the v2 vocabulary is
not a preference writers can ignore; it has to be frozen, and the freeze has
to be mechanical.

The mechanism landed first (2026-08-06; it reached `origin/v2` inside the
squash commit `2d45d84`):
...
This record is numbered `ADR-012`, after `ADR-010` and `ADR-011` landed with
other decisions.

## Decision

**The v2 vocabulary defined in `governance/terminology.yml` is frozen, and
changing it needs a decision record.**
...
4. **Post-freeze vocabulary changes are recorded decisions.** Any change to
   ...
   scanned or pending surface sets needs: an amendment to
...
- The entire v2 rename becomes a red-to-green migration: the house test
  ...
  CI failure rather than a review nit. The pending list reaching empty is,
  by definition, the rename being done.
...
- **Keep the v1 vocabulary.** Rejected: the cold-transcript test is
  unreachable while the framework speaks words it invented, and the cost of
  learning those words lands on every new user forever.
...
- **Gradual rename across ordinary releases, no freeze.** Rejected: each
  ...
  stop later sessions reintroducing retired words - which was the
  predictable failure the freeze was built against.
...
- The freeze's per-slice commits were squashed into `2d45d84` when the
  branch first reached `origin/v2`.
