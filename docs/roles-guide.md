...
The roles are different *assessment* of one document, never different
...
has a real power here: **QA can send an issue back to Specify** if the coverage
...
at the define stage - it becomes an acceptance check and seeds the TDD cycle exactly
...
perspective, applied at the define stage and Plan when the project ships an `architecture/`
directory. (Projects without `architecture/` get the perspective as a no-op; the
load is route-aware.)
...
| Engineer | `/compass:triage` and the pipeline | `delivery-approach.md`, `design.md`, code | Owns Build; Verify's mechanical half | Engineering strategies |
...
reads it at triage: intent is the *actual outcome wanted*, not the literal
...
perspective, at the requirements review and at the pre-Plan gate. The product owner curates the
...
no route removes it. The `marketing-lens` agent applies this perspective and runs
...
The engineer carries the pipeline's spine: Frame (triage), Plan, Build,
...
QA's distinct power is the ability to **send an issue back to Specify** when
...
opens a session, triage reads the role as the fourth dimension, and a
...
Different assessment of one document still have to be reconciled, and the
...
if it changes scope or route, a re-assess.
