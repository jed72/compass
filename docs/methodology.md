...
## 2. The core idea: assess the work, then let the policy route it

Every Compass issue begins with **triage**, which assesses four
...
The assessment is **advisory until confirmed**. The human can override
triage, and a policy floor can override the human (changing a floor means
amending `governance/routing-policy.md`, not overriding one issue's
approach). What no approach is allowed to do is silently cross a
guardrail - see §4.
...
Every delivery approach is the same eight stages in the same order. What
changes between approaches is how much each stage costs, which gates are
enforced, and whether a stage collapses to a single sentence or expands
into a swarm.
...
| **Break down the work** | Git worktrees created; agent swarm assigned, one stream per independent unit. | Skipped entirely on solo work. |
| **Implement** | TDD red → green → refactor (the default strategy). Each agent owns its scenarios. | Test surface scales with risk; the red-before-green strategy is suspended on a spike. |
| **Verify** | BDD scenarios run as acceptance tests; TDD suite runs; review dimensions applied; gates checked. | Number of review dimensions and gates scales with the approach. |
| **Ship** | Worktrees integrated, regression run, living docs updated, owed follow-ups settled. | Integration is trivial for solo, a coordinated merge for swarms. |
...
"done" are never a judgement call. The move from the requirements
review into design is guarded by a
**Definition of Ready** - the spec traces to intent, behaviour is
Given/When/Then, traceability ids are assigned, no open ambiguities, the
approach still fits. The move from verify into shipping is guarded by a
**Definition of Done** - every scenario passes, the suite is green,
coverage meets the guardrail floor, no lint errors, traceability intact.
The checklists live at the foot of `requirements-review.md` and
`verification-report.md` respectively; on approaches where the
requirements review collapses, the Definition of Ready is satisfied by
construction.
...
evidence registry, or `(follow-up: BF-<id>)` referencing an owed follow-up - or
be ticked `[x]` when a human has actually done the work. Bare unchecked items
fail `compass check`'s `dod-evidence-typed` rule. This is the
evidence-not-assertion guardrail expressed at the checklist level: a
written note in a devlog is
...
**The five default guardrails.** These ship active with the framework -
...
4. **Evidence, not assertion.** A guardrail is cleared with artifacts and
   command output, never a claim.
5. **A human signs off on the irreversible.** Changes that can lose data,
...
is the *outcome*: code is tested, acceptance is stated and checkable. The
*form* - Given/When/Then scenarios, red-green-refactor - is the
shipped-on, strong default *way* to reach that outcome. The distinction has
teeth: a one-character typo fix still has to satisfy the guardrail, but
it does not have to perform the full red-before-green ritual to do so;
and the **spike** suspends the TDD strategy entirely so exploration is
not throttled - while the guardrail still applies to anything a spike
graduates into production. Old frameworks made the ritual itself non-negotiable; Compass makes
...
delivery approach, or a human picks. This replaces the older idea of a single supreme
...
If an approach ever appears to require crossing a guardrail, that is a
bug in the approach definition, not a license.
...
`governance/quarantine.yml` - pairs with evidence-not-assertion: the
...
*Writing for a cold reader* extends persistence over conversation by one
step: persistence says put the decision on disk; the cold-reader strategy
says put enough on disk that the next reader does not need the
...
- **Policy floors, caps, and immovable gates** *bound* what triage may
  do. A floor can force an approach to be at least a certain weight
...
the process can flex, what stops it flexing to nothing?"* The policy's
hard rules are what stop it. The flex is real and bounded, and any
`delivery-approach.md` shows not just the approach computed but which
policy rules fired and why. An
...
**judgement**: triage assessing the four dimensions - how risky is
...
On the other side of the line is **mechanism**: everything that happens
once the assessment exists. Composing the candidate shape, applying the
floors and caps, stapling the immovable gates, assembling the gate set -
that is pure function. Same assessment plus same policy produce the same
approach, every time,
...
never checked against outcomes is just assertion with extra steps. So
triage has a feedback loop.

The mechanism is small. When the assessment turns out wrong, the honest
response is a **re-assessment** - re-score the four dimensions
mid-issue - and every one is *recorded*: `compass approach evaluate
--write` detects that the approach changed and appends an entry to the
spine's `reassessments` log, with the reason (`--reason "..."`). One
re-assessment is an anecdote. The log across every issue is data:
`compass retro` aggregates it and reports the pattern - are
re-assessments mostly *up* (triage reads size and risk low - it is
under-sizing) or *down* (it reads risk high - it is over-sizing)? That is
the framework holding a mirror to its own judgement layer. It does not
gate anything; it tells the team whether `routing-policy.yml` or the
sizing rubric in `approaches/rubric.md` needs tuning. The adaptivity is
judgement (§6's first half), and this is how judgement stays honest over
...
Parallelism in Compass is decided at the **design stage** and executed at
**breakdown**.

The design stage produces a **distribution map**: the set of work units,
which are independent, and which share surface area. Independence is
determined from the scenario file and the design - units that touch
disjoint code and satisfy disjoint scenarios can run in parallel.

Breakdown then sets up the topology:

- **Solo** (1 stream) - quick fixes and most feature approaches. No
  worktree; work happens on the current branch. Breakdown is a no-op.
- **Pair** (2–3 streams) - larger feature approaches. One worktree per
  stream.
...
Swarm topology is itself a computed decision: the size and risk
assessment set the default, the distribution map sets the count, and a
policy cap can bound it (e.g. "never swarm a critical-risk change - the
coordination risk outweighs the speed").
...
Approaches are *composed* from the assessment, but in practice most
issues land near one of five reference shapes. These are starting points
triage tunes, not a fixed ladder.

| Shape | Typical assessment | Pipeline shape |
|---|---|---|
| **quick fix** | atomic/small · trivial/contained · brownfield-mapped | triage → define (1 scenario) → implement → verify. The requirements review, design, and breakdown collapse. Still tested before it ships; the red-before-green TDD strategy still applies. One gate. |
| **feature** | standard · contained · either familiarity | Full pipeline, solo or pair. A small scenario set. Governance check at the design stage. Two gates. |
...
| **hotfix** | critical · atomic/small · brownfield | Reproduce-first: a failing regression test *is* the spec. Implementation is expedited, but before the issue closes the approach record is completed properly and the reproduction test is promoted into a real scenario. All verify gates, no exceptions. |
| **spike** | goal is exploration - "I cannot state this well enough yet" | triage (light) → explore (TDD strategy suspended; the hook does not block) → conclude → graduate or discard. **Nothing ships from a spike** - the only exit that keeps code is graduating, which is re-assessing into a real delivery approach where the guardrails apply in full. |

Full definitions, including exact gate sets, live in `approaches/`. The
spike is the escape hatch that keeps the lightweight path honest: it
exists so exploratory work is not forced through a delivery-shaped
pipeline, and it is safe because its de-scopes are all backed by the same
fact - nothing ships from it.
...
Everything above is *issue-centric* - and that is the right default. Each
issue carries its own delivery approach, its own artifacts, its own
gates; the pipeline
...
thing first: guardrail violations (an issue with no
`delivery-approach.md`), approaches quietly outgrown, stalls, and owed
follow-ups aggregated across every issue. It advises;
...
Flow also carries the framework's **retrospective signal**. Every
re-assessment is recorded in the spine's `reassessments` log (§6's
feedback loop), and `compass retro` aggregates that log across every
issue - is triage systematically over- or under-sizing the process? Flow
surfaces that read alongside
...
Every Compass issue begins with triage.  As part of it, the CLI's internal
...
**Why a separate file, not a `task.yml` field?**  The spine's
`assessment` block is *judgement* - triage's reading of the four
dimensions.  Mechanism-produced state (what files exist on disk, their
hashes) does not belong there.  `architecture-loaded.yml` is the
mechanism's output; the assessment is the human's.
...
architecture file changes after triage loaded it, the hash will not match, and
the agent knows to ask triage to reload.  The `parsed` field for
structured
...
exists but is not valid YAML, triage raises an error that names the file
and
...
of traceability (traceability), but they also carry a cross-issue signal: if issue B deletes
...
## 13. Re-assessments - the feedback signal

A **re-assessment** is what happens when triage (or a human) re-reads
the work mid-issue and concludes that the initial approach was wrong.
Re-assessing is normal and expected - it is the mechanism that keeps
process proportionate when reality turns out to differ from first
impressions. What is *not* normal is absorbing a scope change silently,
without filing the re-assessment.

### Why absorbed mis-assessments matter

When a builder discovers mid-implementation that the issue is larger,
narrower, or differently shaped than the design described, and works
around it without filing a re-assessment, two things happen:

1. **The retrospective signal is lost.** `compass retro` reads the
   `reassessments:` log across all issues and reports whether triage is
   systematically over- or under-sizing. An absorbed mis-assessment - a
   real scope change that was not recorded - makes the signal less
   accurate. The pattern repeats; triage never learns.
2. **The audit trail has a gap.** The issue's spine says it was a
   `standard`-sized issue; the devlog says scope ballooned. Anyone
   reading the history cannot reconstruct why the issue took longer than
   the design said.
...
devlog (not nested in quotes or indentation) AND no re-assessment has
been filed
...
The `reassessments:` log answers "did triage mis-*size* the approach?"
The
...
It is captured at **ship time**, when the gap between what triage
promised and what implementation and verify actually cost is freshest.
`compass _friction-capture` (called by the ship procedure) assembles a
draft `friction:` list in the spine from signals the CLI already
computes - recorded re-assessments and absorbed assessment-debt become
`source: derived` entries - and the author may add one
...
5. **One spec, many roles.** The scenario file is the shared substrate for
   every role.
6. **De-scoping is a written decision.** If an approach skips something,
   the
