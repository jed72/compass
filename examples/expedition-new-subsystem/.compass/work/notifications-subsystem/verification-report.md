...
> **Route (from route.md):** Expedition · **Gate count:** all (full set)
...
|---|---|---|---|
| SCN-001 | An in-app event produces a notification for the target user | PASS | §2 (stream-1) |
| SCN-002 | A notification is delivered once, even if the event is retried | PASS | §2 (stream-1) |
| SCN-003 | Notifications survive a worker restart | PASS | §2 (stream-1) |
| SCN-004 | A user mutes a category and stops receiving that category | PASS | §2 (stream-2) |
| SCN-005 | A user with no saved preferences gets the safe defaults | PASS | §2 (stream-2) |
| SCN-006 | A muted category does not suppress a security notification | PASS | §2 (stream-2) |
...
| governance | always | PASS | G1: every scenario has a passing test it traces to (§2). G2: all six stated and Clarify-closed before Build. G3: see traceability. G4: every gate below has a resolving evidence pointer. **G5: GREEN** — `migrations/0042` signed off by L. Haddad (eng lead) before Land, recorded in `task.yml` `approvals:`; forward and rollback paths reviewed. S2 red-before-green followed in both worktrees. |
...
| clarity | yes | PASS | Four focused modules, one responsibility each; `plan.md` DD-1/2/3 record the three non-obvious choices (fixed security category, once-ever dedup, write-before-deliver durability) for a future reader. The two-stream split is documented in `distribution-map.md`. |
| claims | yes (in Expedition's shape) | PASS | No product-marketer was in play — there are no public claims to back, and `task.yml` `claims:` is empty. The gate exists in Expedition's set and is satisfied trivially: `compass check`'s claim-traces-to-scenario passes because there is nothing unbacked. The external launch is a separate later task. |
...
| verify.claims | route (Expedition shape) | GREEN (no claims to check — satisfied trivially) |
