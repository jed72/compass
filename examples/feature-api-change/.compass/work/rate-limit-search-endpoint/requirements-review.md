...
> **Clarify weight (from route.md):** light pass

---

## Self-QA of the spec

- Every scenario has an observable `Then` - checked. SCN-002's "the search query
  is not executed" is observable via the query-count assertion in the harness,
  not a wish.
- No two scenarios contradict each other. SCN-001 and SCN-002 share a Given
  ("limit is 100/min") and differ only in the client's prior count - that is a
  boundary pair, not a contradiction.
- One untestable phrase found and fixed: an earlier draft of SCN-003 said the
...
- **Question:** SCN-004 says "the window rolls over". A fixed window and a
  sliding window both "roll over" but behave differently at the boundary - the
  spec must commit to one or SCN-004 is not testable.
- **Resolution:** Fixed window. The incident did not involve boundary-burst
  abuse, and a fixed window is simpler to reason about and to test. SCN-004's
...
- **Spec change:** SCN-004 Given edited.
- **Status:** resolved
...
- **Spec change:** no spec change, clarification only. Recorded in `plan.md` DD-2.
- **Status:** resolved
...
- [x] `spec.feature.md` updated to reflect every resolution (SCN-004 Given tightened; Q2 needed no scenario).
- [x] Non-engineering roles in play have reviewed - n/a, this is an engineer-only task.
...
- [x] **Traceability ids assigned** - SCN-001…SCN-005, all present in
      `spec.feature.md` and `task.yml`.
- [x] **Affected surface named** - `route.md` §4 and the upcoming `plan.md` §4
