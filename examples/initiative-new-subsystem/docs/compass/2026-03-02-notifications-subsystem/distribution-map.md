...
> **Stage:** plan · **Date:** 2026-03-07 · **Reads from:** technical-design.md §4, acceptance-criteria.md
...
|---|---|---|---|
| U1 ↔ U2 | mostly - overlap only on `api.py` (different endpoints) and `migrations/0042` (the U0 foundation) | yes - group A vs group B, no shared `TRC-id` | independent - the bounded `api.py` overlap is what the orchestrator watches, not a reason to fold |
...
- One worktree + one `builder` per subtask, plus one `orchestrator` that writes
  ...
  implement, and integrates at ship.
...
<!-- 2 subtasks is below the "4+" that the word "multiagent" usually implies, but it
     ...
     orchestrator) would leave that shared surface unwatched. The orchestration
...
- **Final subtask count after caps:** 2 - unchanged, no cap applied.
...
- [x] Final subtask count respects every cap in §5 - no routing cap in play.
