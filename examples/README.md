...
Five completed Compass tasks, one per reference route. Each is a real-shaped
`.compass/work/<task-slug>/` directory exactly as it would look *after the task
...
Each example is self-contained and verifiable. From inside any example's task
directory:
...
### 1. `express-typo/` - the Express route

**Task:** the upload timeout error says "try again later" when it should name
the file-size limit that is the actual cause.

**What it demonstrates:** the *lightness*. Clarify, Plan, and Distribute all
collapse - and `route.md` §5 carries an explicit "safe to skip because…" line
...
*correct* - Express adapts test surface, never test existence, so `red.json`
...
### 2. `standard-api-change/` - the Standard route

**Task:** add per-client rate limiting to the public `/search` endpoint after
...
`clarifications.md` with the Definition of Ready ticked, a real `plan.md` with
...
### 3. `hotfix-regression/` - the Hotfix route

**Task:** `/search` returns a 500 for any request with an empty `filter` object -
...
backfill**. `task.yml`'s `backfills:` lists three owed items; `route.md` §6 and
the `devlog.md` Land entry show all three **paid** - which is what makes this a
closed task and not an open one. `compass check` enforces it.

**One-line lesson:** speed is borrowed from the front of the pipeline and paid
back at the end - a Hotfix with an unpaid backfill is an open task, full stop.

### 4. `expedition-new-subsystem/` - the Expedition route

**Task:** build the in-app notifications subsystem - durable delivery,
...
and two routing guardrails *firing visibly*. The readings include
`touches: [migrations]`, so **RG-FLOOR-003** raises the candidate from Standard
to Expedition ("domain risk overrides magnitude") - you can see it in
`task.yml`'s `fired_guardrails` and in `route.md` §3. A product owner is
involved, so **RG-ROLE-002** requires `brief.md` and blocks Plan until the spec
...
at lives under `evidence/approval-migration-0042.json`. Full set: `brief.md`
...
magnitude reading, and the heavy route is heavy on purpose.

### 5. `spike-technical-unknown/` - the Spike route

**Task:** is `weasyprint` a viable PDF engine for our report layouts, or do we
...
If you read them in route-weight order - `express-typo`, `standard-api-change`,
`hotfix-regression`, `expedition-new-subsystem`, `spike-technical-unknown` -
