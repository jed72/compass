...
> **Assessed:** 2026-05-11 by P. Adeyemi (engineer) · **Revision:** 1
...
| **Size:** | small | The fix is a one-branch null/empty guard in `filter_compiler.py`. A large fix would have made this an initiative under incident command. It is small. |
...
Hotfix is selected by **urgency**, not by the size/risk/familiarity
composition - `urgency: live-defect` with `size: small` matched routing
policy rule RP-SHAPE-002 ("a live defect, small enough to reproduce-and-fix
fast"). The assessment still scored all four dimensions; they shape the
follow-up and confirm this is a hotfix and not an incident.
...
`compass approach evaluate` confirmed it. The rule that *did not* fire is worth
...
changes coordinate, or they break things quietly"), because a critical-risk
production defect is an incident with incident command, not a solo hotfix. The
assessment kept risk at `cross-cutting` deliberately: wide, but no data or
trust at stake.
...
| Implement | Expedited | The reproduction test is already red; make it green with the smallest correct guard; no refactor (the branch was already minimal). |
| Verify | Full | **Not compressed.** All five Hotfix gates, evidence pasted - see `verification-report.md`. |
| Ship | Full + follow-up | Ship the fix, then finish the mandatory follow-up - §6. |
...
- Immovable gates added: verify.correctness, verify.governance, verify.traceability - all already in Hotfix's shape. `verify.regression` is part of Hotfix's own set: a fast fix that breaks something else is just a faster outage.
