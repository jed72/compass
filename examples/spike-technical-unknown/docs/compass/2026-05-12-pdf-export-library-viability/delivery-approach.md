...
> **Assessed:** 2026-05-12 by N. Brandt (engineer) · **Revision:** 1
> **Reference shape:** Spike

<!-- On a spike assess also writes a `.spike` marker file in this
...
| **Goal & role** | engineer / **exploration** | An engineer who cannot yet frame the real feature - the open question ("is this library viable?") has to be answered before a delivery approach can be composed. Exploration intent is what selects Spike. |

**Domain tags (`labels:`):** none - and this matters for a spike. A
Spike may not touch anything irreversible (auth, payments, personal-data,
migrations); those are raised to initiative by a floor rule regardless of intent. This
...
Spike is selected by **goal**, the way Hotfix is selected by urgency.
`goal: exploration` matched routing policy rule RP-SHAPE-001 ("exploration
intent - cannot frame for delivery yet"). The assessment still scored the other
...
- **RP-FLOOR-003** (`labels_any: [auth, payments, personal-data, migrations]`)
  did not fire - `labels:` is empty. Had this question required touching any
...
| Implement | = **Explore** | Write rendering code freely to answer the question. **TDD strategy (`S2`) suspended** - the pre-tool hook reads the delivery approach and does not block edits here. Code is assumed throwaway. |
...
- A note on the immovable gates: for this issue, `manifest.yml`'s `gates:` list
  carries only `spike.conclude`. The delivery-only immovable gates
  (`verify.correctness`, `verify.governance`, `verify.traceability`) are not
  seeded on a spike at all - a spike ships nothing, so there is nothing for
  them to be about.
...
| Implement - TDD strategy | suspended | Red-before-green is the wrong discipline for throwaway learning code. `G1` is not skipped - it is **deferred to graduation**, where it applies in full. **Nothing lands from a spike.** |
...
- **Timebox:** 2 days. When the clock runs out: conclude with what is known, or
  re-assess the spike with a new timebox and a written reason. No silent
  overrun.
...
- [x] Every skipped/collapsed stage in §5 has a "safe to skip because…" line (all the same fact: nothing lands).
