...
Then('the reset succeeds', function () { assert.ok(this.result.ok, this.result.error); });
...
Then('the password change is recorded', function () { assert.strictEqual(this.changes.length, 1); });

Then('no password change is recorded', function () { assert.deepStrictEqual(this.changes, []); });
