...
`compass bdd extract` tags every scenario with its traceability id, prefixed
`@` (for example `TRC-A1`), which is what lets a per-scenario runner result
map back to `manifest.yml`.
pytest-bdd turns each tag into a pytest mark - and pytest warns about any mark
it has not been told about, so without this an adopter's first green run
prints a `PytestUnknownMarkWarning` for every tag.
...
alongside the wiring; it is the one piece of wiring that is not obvious.
"""
