...
def _project(tmp_path, task_yml):
    ...
    (work / "manifest.yml").write_text(task_yml, encoding="utf-8")
...
def test_lint_reports_a_bare_string_scenario_instead_of_crashing(tmp_path):
    ...
    assert "Traceback" not in combined, (
        f"`task lint` crashed on a malformed manifest.yml:\n{combined}"
    )
    assert result.returncode != 0, "a malformed manifest.yml must fail the lint"
