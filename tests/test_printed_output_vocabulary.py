...
def test_trc_b3_spine_keys_unchanged(tmp_path):
    """Translating at the print boundary must not move a single key.

    Renaming the stage keys needs a back-compat shim for every spine already
    on disk, and is the rename slice's work.
    """
    root = tmp_path / "proj"
    ...
    assert set(spine["stages"]) == {
        "frame", "specify", "clarify", "plan", "distribute", "build",
        "verify", "land"}, (
        f"the stage KEYS moved - that is the rename slice's work, not this "
        f"issue's: {sorted(spine['stages'])}")
